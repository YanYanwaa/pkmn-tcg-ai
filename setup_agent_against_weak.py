...
# --- Debug logging ---
DEBUG_OPTIONS = True  # flip to True for a single-game debug run; keep False for batch runs
...
def agent(obs_dict: dict) -> list[int]:
    ...
    for opt_idx, o in enumerate(select.option):
        ...
        if o.type == OptionType.CARD:
            ...
            elif has_meganium and bench_attacker_ready and not active_is_attacker and not ex_wall_is_active:  # <-- CHANGED
    ...
    if attack_cache:
        ...
        for idx, attack_value in attack_cache.items():
            if best_other > base_score:
                ...
                scores[idx] = base_score - 1 + attack_value
